...
// TODO - #1: Add support to the 'filter' endpoint for a new query parameter 'letter'
// which filters friends by starting letter

// TODO - #2: Modify the 'info' route to only return the user-agent, content-type
...
// TODO - #5: Move all logic out into a controller with functions for
// finding, filtering, info, adding and updating
...
//ie. /friends/filter?gender=male

// 1. Add support to also filter by a starting 'letter' query parameter
// ie. /friends/filter?letter=R
...
router.get("/filter", (req, res) => {
  ...
  if (filterLetter) {
    matchingFriends = matchingFriends.filter((friend) =>
      friend.name
        .toLocaleLowerCase()
        .startsWith(filterLetter.toLocaleLowerCase())
    );
  }
  if (matchingFriends.length > 0) {
    ...
    res.status(404).json({ error: "No friends these criteria" });
  }
});
...
router.get("/:id", (req, res) => {
  ...
  let matchingFriend = [...friends];
  let foundFriend = matchingFriend.find((item) => item.id == friendId);
  ...
  if (foundFriend) {
    ...
    res
      .status(404)
      .json({ message: `Friend matching ID: ${friendId} not found` });
  }
});
...
router.put("/:id", (req, res) => {
  let friendId = parseInt(req.params.id, 10); // Ensure friendId is a number
  let updatedFriend = req.body;

  if (!updatedFriend.name || !updatedFriend.gender) {
    res
      .status(500)
      .json({ message: "Friend object must contain a name and gender" });
    return;
  }
  ...
  if (friendIndex !== -1) {
    friends[friendIndex] = { ...friends[friendIndex], ...updatedFriend }; // Merge old and new data

    res.json({
      message: "Object updated successfully",
      updatedObject: friends[friendIndex],
    });
  } else {
